...
class Mcp_Meta_Box_types {
	...
	/**
	 * Default fallback. Allows rendering fields via "mcp_render_$name" hook
	...
	public function __call( $name, $arguments ) {
		...
		do_action( "mcp_render_$name", $this->field->args(), $this->field->escaped_value(), $this->field->object_id, $this->field->object_type, $this );
	}
	...
	public function get_object_terms() {
		...
		if ( ! $post = get_post( $object_id ) ) {
			...
			$cache_key = 'mcp-cache-'. $taxonomy .'-'. $object_id;
	...
	public function is_valid_img_ext( $file ) {
		...
		$this->valid = empty( $this->valid )
			? (array) apply_filters( 'mcp_valid_img_types', array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ) )
			: $this->valid;
	...
	public function parse_args( $args, $element, $defaults ) {
		return wp_parse_args( apply_filters( "mcp_{$element}_attributes", $this->field->maybe_set_attributes( $args ), $this->field, $this ), $defaults );
	}
	...
	public function render_repeatable_field() {
		...
		<table id="<?php echo $table_id; ?>" class="mcp-repeat-table">
			<tbody>
				<?php $this->repeatable_rows(); ?>
			</tbody>
		</table>
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php _e( 'Add Row', 'mcp-custom-post' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php _e( 'Remove', 'mcp-custom-post' ); ?></a>
			</td>
		</tr>
	...
	public function _desc( $paragraph = false, $echo = false ) {
		...
		$desc = "\n<$tag class=\"mcp_metabox_description\">{$this->field->args( 'description' )}<$tag>\n";
		if ( $echo )
			echo $desc;
		return $desc;
	}
	...
	public function icon_picker( $args = array() ) {
		...
		return sprintf( '<div class="input-group iconpicker-container mcp-post-icon-picker">
	...
	public function text_small() {
		return $this->input( array( 'class' => 'mcp_text_small', 'desc' => $this->_desc() ) );
	}

	public function text_medium() {
		return $this->input( array( 'class' => 'mcp_text_medium', 'desc' => $this->_desc() ) );
	}

	public function text_email() {
		return $this->input( array( 'class' => 'mcp_text_email mcp_text_medium', 'type' => 'email' ) );
	}

	public function text_url() {
		return $this->input( array( 'class' => 'mcp_text_url mcp_text_medium regular-text', 'value' => $this->field->escaped_value( 'esc_url' ) ) );
	}

	public function text_date() {
		return $this->input( array( 'class' => 'mcp_text_small mcp_datepicker', 'desc' => $this->_desc() ) );
	}

	public function text_time() {
		return $this->input( array( 'class' => 'mcp_timepicker text_time', 'desc' => $this->_desc() ) );
	}

	public function text_money() {
		return ( ! $this->field->args( 'before' ) ? '$ ' : ' ' ) . $this->input( array( 'class' => 'mcp_text_money', 'desc' => $this->_desc() ) );
	}

	public function textarea_small() {
		return $this->textarea( array( 'class' => 'mcp_textarea_small', 'rows' => 4 ) );
	}

	public function textarea_code() {
		return sprintf( '<pre>%s</pre>', $this->textarea( array( 'class' => 'mcp_textarea_code' )  ) );
	}
	...
	public function text_date_timestamp() {
		...
		return $this->input( array( 'class' => 'mcp_text_small mcp_datepicker', 'value' => $value ) );
	}
	...
	public function text_datetime_timestamp( $meta_value = '' ) {
		...
		$inputs = array(
			$this->input( array(
				'class' => 'mcp_text_small mcp_datepicker',
			...
			$this->input( array(
				'class' => 'mcp_timepicker text_time',
	...
	public function colorpicker() {
		...
		return $this->input( array( 'class' => 'mcp_colorpicker mcp_text_small', 'value' => $meta_value ) );
	}

	public function title() {
		extract( $this->parse_args( array(), 'title', array(
			'tag'   => $this->field->object_type == 'post' ? 'h5' : 'h3',
			'class' => 'mcp_metabox_title',
	...
	public function multicheck( $type = 'checkbox' ) {
		return $this->radio( array( 'class' => 'mcp_checkbox_list mcp_list', 'options' => $this->concat_options( array( 'type' => 'checkbox', 'name' => $this->_name() .'[]' ), 'list_input_checkbox' ) ), $type );
	}
	...
	public function checkbox() {
		$meta_value = $this->field->escaped_value();
		$args = array( 'type' => 'checkbox', 'class' => 'mcp_option mcp_list', 'value' => 'on', 'desc' => '' );
		if ( ! empty( $meta_value ) ) {
			$args['checked'] = 'checked';
		}
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'mcp-custom-post' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function taxonomy_multicheck() {
		...
		return $this->radio( array( 'class' => 'mcp_checkbox_list mcp_list', 'options' => $options ), 'taxonomy_multicheck' );
	}
	...
	public function file_list() {
		...
		echo $this->input( array(
			'type'  => 'hidden',
			'class' => 'mcp_upload_file mcp_upload_list',
			'size'  => 45, 'desc'  => '', 'value'  => '',
		) ),
		$this->input( array(
			'type'  => 'button',
			'class' => 'mcp_upload_button button mcp_upload_list',
			'value'  => __( 'Add or Upload File', 'mcp-custom-post' ),
			'name'  => '', 'id'  => '',
		) );

		echo '<ul id="', $this->_id( '_status' ) ,'" class="mcp_media_status attach_list">';
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="mcp_remove_wrapper"><a href="#" class="mcp_remove_file_button">'. __( 'Remove Image', 'mcp-custom-post' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						__( 'File:', 'mcp-custom-post' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'mcp-custom-post' ) .'</a> / <a href="#" class="mcp_remove_file_button">'. __( 'Remove', 'mcp-custom-post' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		echo $this->input( array(
			'type'  => $input_type,
			'class' => 'mcp_upload_file',
			'size'  => 45,
			'desc'  => '',
		) ),
		'<input class="mcp_upload_button button" type="button" value="'. __( 'Add or Upload File', 'mcp-custom-post' ) .'" />',
		$this->_desc( true );
		...
		$this->field = new Mcp_Meta_Box_field( $args, $this->field->group );
		...
		if ( $meta_value && ! $_id_value ) {
			$_id_value = Mcp_Meta_Box::image_id_from_url( esc_url_raw( $meta_value ) );
		}

		echo $this->input( array(
			'type'  => 'hidden',
			'class' => 'mcp_upload_file_id',
			'value' => $_id_value,
			'desc'  => '',
		) ),
		'<div id="', $this->_id( '_status' ) ,'" class="mcp_media_status">';
			if ( ! empty( $meta_value ) ) {

				if ( $this->is_valid_img_ext( $meta_value ) ) {
					echo '<div class="img_status">';
					echo '<img style="max-width: 350px; width: 100%; height: auto;" src="', $meta_value, '" alt="" />';
					echo '<p class="mcp_remove_wrapper"><a href="#" class="mcp_remove_file_button" rel="', $cached_id, '">'. __( 'Remove Image', 'mcp-custom-post' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'mcp-custom-post' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. __( 'Download', 'mcp-custom-post' ) .'</a> / <a href="#" class="mcp_remove_file_button" rel="', $cached_id, '">'. __( 'Remove', 'mcp-custom-post' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public function oembed() {
		echo $this->input( array(
			'class'           => 'mcp_oembed regular-text',
			'data-objectid'   => $this->field->object_id,
			'data-objecttype' => $this->field->object_type
		) ),
		'<p class="mcp-spinner spinner" style="display:none;"><img src="'. admin_url( '/images/wpspin_light.gif' ) .'" alt="spinner"/></p>',
		'<div id="',$this->_id( '_status' ) ,'" class="mcp_media_status ui-helper-clearfix embed_wrap">';

			if ( $meta_value = $this->field->escaped_value() ) {
				echo Mcp_Meta_Box_ajax::get_oembed( $meta_value, $this->field->object_id, array(
