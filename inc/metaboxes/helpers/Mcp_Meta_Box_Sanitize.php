...
class Mcp_Meta_Box_Sanitize {

	/**
	 * A MCP field object
	 * @var Mcp_Meta_Box_field object
	 */
	public $field;
	 ...
	 * @param object $field A MCP field object
	...
	public function default_sanitization( $value ) {
		...
		$updated = apply_filters( 'mcp_validate_'. $this->field->type(), null, $value, $this->object_id, $this->field->args(), $this );
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = Mcp_Meta_Box::timezone_string();

		$offset = Mcp_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new Mcp_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = Mcp_Meta_Box::image_id_from_url( $value );
		}
