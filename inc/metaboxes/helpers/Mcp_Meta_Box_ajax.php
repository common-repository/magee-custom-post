...
class Mcp_Meta_Box_ajax {
	 ...
	 * @return Mcp_Meta_Box_ajax A single instance of this class.
	 */
	...
	public function oembed_handler() {
		...
		// verify our nonce
		if ( ! ( isset( $_REQUEST['mcp_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['mcp_ajax_nonce'], 'ajax_nonce' ) ) )
			die();
		...
		if ( empty( $oembed_string ) )
			self::send_result( '<p class="ui-state-error-text">'. __( 'Please Try Again', 'mcp-custom-post' ) .'</p>', false );
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'Mcp_Meta_Box_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'Mcp_Meta_Box_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p class="mcp_remove_wrapper"><a href="#" class="mcp_remove_file_button" rel="'. $args['field_id'] .'">'. __( 'Remove Embed', 'mcp-custom-post' ) .'</a></p></div>';

		// Otherwise, send back error info that no oEmbeds were found
		return '<p class="ui-state-error-text">'. sprintf( __( 'No oEmbed Results Found for %s. View more info at', 'mcp-custom-post' ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
	...
	public static function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		$data = 'options-page' === self::$object_type
			? Mcp_Meta_Box::get_option( self::$object_id, self::$embed_args['cache_key'] )
	...
	public static function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( 'options-page' === self::$object_type ) {
			// Set the option
			Mcp_Meta_Box::update_option( self::$object_id, self::$embed_args['cache_key'], $meta_value, array( 'type' => 'oembed' ) );
			// Save the option
			Mcp_Meta_Box::save_option( self::$object_id );
		} else {
