...
spl_autoload_register('Mcp_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'mcp_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new Mcp_Meta_Box( $meta_box );
}

define( 'MCP_META_BOX_URL', Mcp_Meta_Box::get_meta_box_url() );
...
class Mcp_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'mcp_styles' => true, // Include mcp bundled stylesheet
		'fields'     => array(),
	);
	...
	/**
	 * Whether MCP nonce has been added to the page. (oly add once)
	 * @var   bool
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'mcp_allow_frontend', true, $meta_box );
		...
		$show_filters = 'Mcp_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'mcp_show_on', array( $show_filters, $filter ), 10, 2 );
		}

		// register our scripts and styles for mcp
	...
	/**
	 * Registers scripts and styles for MCP
	...
	public function register_scripts() {
		...
		// scripts required for mcp
		$scripts = array( 'jquery', 'jquery-ui-core', 'mcp-datepicker', /*'media-upload', */'mcp-timepicker' );
		// styles required for mcp
		$styles = array();
		...
		wp_register_script( 'mcp-datepicker', MCP_META_BOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'mcp-timepicker', MCP_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'mcp-scripts', MCP_META_BOX_URL .'js/mcp.js', $scripts, '' );

		wp_enqueue_media();

		wp_localize_script( 'mcp-scripts', 'mcp_l10', apply_filters( 'mcp_localized_data', array(
			'ajax_nonce'      => wp_create_nonce( 'ajax_nonce' ),
			'script_debug'    => defined('SCRIPT_DEBUG') && SCRIPT_DEBUG,
			'new_admin_style' => version_compare( $wp_version, '3.7', '>' ),
			'object_type'     => self::get_object_type(),
			'upload_file'     => __( 'Use this file', 'mcp-custom-post' ),
			'remove_image'    => __( 'Remove Image', 'mcp-custom-post' ),
			'remove_file'     => __( 'Remove', 'mcp-custom-post' ),
			'file'            => __( 'File:', 'mcp-custom-post' ),
			'download'        => __( 'Download','mcp-custom-post' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All', 'mcp-custom-post' ),
		) ) );

		wp_register_style( 'mcp-styles', MCP_META_BOX_URL . 'style'. $min .'.css', $styles );
	...
	/**
	 * Enqueues scripts and styles for MCP
	 
	 */
	public function do_scripts( $hook ) {
		// only enqueue our scripts/styles on the proper pages
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			wp_enqueue_script( 'mcp-scripts' );

			// default is to show mcp styles on post pages
			if ( $this->_meta_box['mcp_styles'] )
				wp_enqueue_style( 'mcp-styles' );
		}
	}
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'mcp_show_on', true, $this->_meta_box ) )
	...
	public function user_metabox() {
		...
		// default is to NOT show mcp styles on user profile page
		if ( $this->_meta_box['mcp_styles'] != false )
			wp_enqueue_style( 'mcp-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		echo "\n<!-- Begin MCP Fields -->\n";
		do_action( 'mcp_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table mcp_metabox">';
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new Mcp_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
		do_action( 'mcp_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End MCP Fields -->\n";
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new Mcp_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
					echo '<h2 class="mcp-group-name">'. $label .'</h2>';
				if ( $desc )
					echo '<p class="mcp_metabox_description">'. $desc .'</p>';
			echo '</th></tr>';
		}
	...
	public static function render_group_row( $field_group, $remove_disabled ) {
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				<table class="mcp-nested-table" style="width: 100%;">';
				if ( $field_group->options( 'group_title' ) ) {
					echo '
					<tr class="mcp-group-title">
						<th colspan="2">
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new Mcp_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'mcp_show_on', true, $meta_box ) )
			return;
		...
		do_action( "mcp_save_{$object_type}_fields", $object_id, $meta_box['id'], self::$updated, $meta_box );
	 ...
	 * @return string URL to MCP resources
	 */
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$mcp_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );
		...
		$mcp_url = set_url_scheme( $mcp_url );

		return trailingslashit( apply_filters('mcp_meta_box_url', $mcp_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'mcp_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "mcp_override_option_get_$option_key", 'mcp_no_override_option_get', $default );

		if ( $test_get !== 'mcp_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'mcp_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "mcp_override_option_save_$option_key", 'mcp_no_override_option_save', $to_save );

		if ( $test_save !== 'mcp_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_mcp_oembed_handler', array( 'Mcp_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_mcp_oembed_handler', array( 'Mcp_Meta_Box_ajax', 'oembed_handler' ) );

/**
 * A helper function to get an option from a MCP options array
 
 * @param  string  $option_key Option key
 * @param  string  $field_id   Option array field key
 * @return array               Options array or specific field
 */
function mcp_get_option( $option_key, $field_id = '' ) {
	return Mcp_Meta_Box::get_option( $option_key, $field_id );
}

/**
 * Get a MCP field object.
 
 * @param  array  $field_args  Field arguments
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              Mcp_Meta_Box_field object
 */
function mcp_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	Mcp_Meta_Box::set_object_id( $object_id );
	Mcp_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new Mcp_Meta_Box_field( $field_args );
}
 ...
 */
function mcp_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = mcp_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function mcp_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		mcp_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function mcp_print_metabox( $meta_box, $object_id ) {
	$mcp = new Mcp_Meta_Box( $meta_box );
	if ( $mcp ) {

		Mcp_Meta_Box::set_object_id( $object_id );

		if ( ! wp_script_is( 'mcp-scripts', 'registered' ) )
			$mcp->register_scripts();

		wp_enqueue_script( 'mcp-scripts' );

		// default is to show mcp styles
		if ( $meta_box['mcp_styles'] != false )
			wp_enqueue_style( 'mcp-styles' );

		Mcp_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function mcp_save_metabox_fields( $meta_box, $object_id ) {
	Mcp_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 * @return string             MCP html form markup
 */
function mcp_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = Mcp_Meta_Box::set_mb_defaults( $meta_box );
	...
	Mcp_Meta_Box::set_object_type( Mcp_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		// check nonce
		isset( $_POST['submit-mcp'], $_POST['object_id'], $_POST['wp_meta_box_nonce'] )
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], Mcp_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		mcp_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'mcp_frontend_form_format', '<form class="mcp-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-mcp" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );
